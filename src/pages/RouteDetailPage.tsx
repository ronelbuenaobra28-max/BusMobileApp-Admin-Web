...
export default function RouteDetailPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="p-6">
          <h3 className="mb-4 text-lg font-semibold text-slate-900">Assigned Operators</h3>
          {!route.operators || route.operators.length === 0 ? (
            <p className="text-sm text-slate-500">No operators assigned to this route.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {route.operators.map((op) => (
                <span
                  key={op.operator_id}
                  className="inline-flex items-center rounded-md border px-2.5 py-1 text-xs font-medium border-transparent bg-slate-100 text-slate-700"
                >
                  {op.operator_name}
                </span>
              ))}
                ...
                <tbody>
                  {routeTrips.map((t) => (
                    <tr key={t.id} className="border-b border-slate-100">
                      ...
                      <td className="px-3 py-2 text-slate-700">{t.operator_name || "—"}</td>
                      <td className="px-3 py-2 text-slate-700">{t.bus_number || "—"}</td>
